...
const importCloudWorkspace = makeFunctionReference<"mutation", { projects: WorkspaceProject[] }, CloudImportResult>("relayWorkspaceImport:importLocalWorkspace");
...
export function useCloudWorkspaceBackupPort(): WorkspaceBackupPort {
  ...
  return useMemo(() => ({
    ...
    async applyBackup(backup: RelayWorkspaceBackup) {
      try {
        const result = await runImport({ projects: backup.workspace.projects });
