...
export type LocalWorkspaceState = { clients: RelayClient[]; projects: WorkspaceProject[]; workflowTemplates?: WorkflowTemplate[]; projectGroups?: ProjectGroup[]; projectOutputs?: ProjectOutput[] };
...
export function readLocalWorkspaceState(storage: Pick<Storage, "getItem">): LocalWorkspaceState | null {
  try {
    ...
    return Array.isArray(state.clients) && Array.isArray(state.projects) && (state.workflowTemplates === undefined || Array.isArray(state.workflowTemplates)) && (state.projectGroups === undefined || Array.isArray(state.projectGroups)) && (state.projectOutputs === undefined || Array.isArray(state.projectOutputs)) ? state : null;
  } catch { return null; }
}
