...
This review records which external providers are trusted enough to model in CutLab and what must be true before any live OAuth, API synchronization, webhook delivery, accounting sync, or payment collection is implemented.

Current implementation status: CutLab only stores provider-neutral links, provider IDs, local invoice draft CSVs, and Convex/Clerk-backed app data. It does not exchange OAuth tokens with Google Drive, Dropbox, Slack, or Frame.io, does not send Slack messages, does not sync provider files through external APIs, and does not collect payments.
...
- User-facing copy clearly identifies what data leaves CutLab and which provider receives it.
...
| Provider | Current CutLab Scope | Trust Evidence | Live Integration Decision |
| --- | --- | --- | --- |
